...
{ // 在此补充本段代码,如果从键盘输入时，在屏幕给出提示，否则直接提取数据
    if (&in == &cin)
    {
        cout << "依次输入商品代号，商品名称，价格和数量，用空格分割，回车符确认：" << endl;
...
{                                                          // 在此补充本段代码
    value += m.value;
    int tempcount1 = nums + m.nums;
    Goods *templist = new Goods[tempcount1];
    for (int i = 0; i < nums; i++)
    {
        templist[i] = gs[i];
    }
    ...
    {
        templist[i + nums] = m.gs[i];
    }

    Goods EmptyGoods("-0000");
    int rightcount = tempcount1;

    for (int i = 0; i < tempcount1 - 1; i++)
    {
        for (int j = i + 1; j < tempcount1; j++)
        {
            if (templist[i].id == "-0000")
                continue;
            if (templist[j].id == templist[i].id)
            {
